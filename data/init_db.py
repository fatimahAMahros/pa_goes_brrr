...
def insert_comments_bulk(rows: list[dict]):
    """
    Insert many comments at once. If the instagram_comment_id already exists, 
    it automatically updates the clean_text and tokens with the newest preprocessing version.
    """
    conn = get_connection()
    cur = conn.cursor()
    cur.executemany("""
        INSERT INTO comments (instagram_comment_id, month, comment_date, post_id, raw_text, clean_text, tokens)
...
def upsert_clustering_run(month, scenario, linkage, optimal_k, status, dominant_threshold=None, accuracy=None, ari_score=None, notes=None, is_recommended=0):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO clustering_runs (month, scenario, linkage, optimal_k, status, dominant_threshold, accuracy, ari_score, notes, is_recommended)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(month, scenario, linkage) DO UPDATE SET
            ...
            ari_score = excluded.ari_score,
            notes = excluded.notes,
            is_recommended = excluded.is_recommended,
            created_at = datetime('now')
    """, (month, scenario, linkage, optimal_k, status, dominant_threshold, accuracy, ari_score, notes, is_recommended))
    conn.commit()
    row_id = cur.execute("SELECT id FROM clustering_runs WHERE month=? AND scenario=? AND linkage=?", (month, scenario, linkage)).fetchone()[0]
    conn.close()
    return row_id

def upsert_clustering_run(month, scenario, linkage, optimal_k, status, dominant_threshold=None, accuracy=None, evaluation_score=None, notes=None, is_recommended=0):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO clustering_runs (month, scenario, linkage, optimal_k, status, dominant_threshold, accuracy, evaluation_score, notes, is_recommended)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(month, scenario, linkage) DO UPDATE SET
            optimal_k = excluded.optimal_k,
            status = excluded.status,
            ...
            is_recommended = excluded.is_recommended,
            created_at = datetime('now')
    """, (month, scenario, linkage, optimal_k, status, dominant_threshold, accuracy, evaluation_score, notes, is_recommended))
    conn.commit()
    row_id = cur.execute("SELECT id FROM clustering_runs WHERE month=? AND scenario=? AND linkage=?", (month, scenario, linkage)).fetchone()[0]
    conn.close()
    return row_id
