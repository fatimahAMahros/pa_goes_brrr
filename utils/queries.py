...
def get_dataset_overview() -> dict:
    """
    Mengambil statistik dasar dari dataset komentar.
    Menggunakan kolom 'comment_date' sesuai skema untuk mengambil rentang waktu.
    """
    conn = _conn()
    cursor = conn.cursor()
    ...
    try:
        ...
        # Karena _conn() menggunakan sqlite3.Row, kita harus memanggilnya dengan key (string)
        total_rows = row["total_rows"] if row["total_rows"] else 0
        
        # Memotong string "YYYY-MM-DD HH:MM:SS" menjadi "YYYY-MM-DD"
        min_date = row["min_date"][:10] if row["min_date"] else "-" 
        max_date = row["max_date"][:10] if row["max_date"] else "-"
        ...
        print(f"Error mengambil overview: {e}")
        return {"total_rows": 0, "min_date": "-", "max_date": "-"}
    finally:
        conn.close()
